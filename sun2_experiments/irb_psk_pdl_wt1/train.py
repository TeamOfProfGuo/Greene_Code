...
class Trainer():
    def __init__(self, args):
        ...
        trainset = SUNRGBD(transform=transforms.Compose([scaleNorm(),
                                                           ...
                                                           Normalize()]),
                             phase_train=True)
        self.trainloader = data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, **load_kwargs)   # drop_last=True pin_mem
        ...
        self.valloader = data.DataLoader(testset, batch_size=args.batch_size, drop_last=False, shuffle=False, **load_kwargs)  # drop_last=True pin_mem
        ...
        if train_args['class_weight'] is not None:
            ...
            if os.path.exists(wt_path):
                ...
                wt = trainset.compute_class_weights(weight_mode=train_args['class_weight'] )
        ...
        self.scheduler = utils.LR_Scheduler_Head(args.lr_scheduler, args.lr, args.epochs,
                                                 iters_per_epoch=len(self.trainloader), warmup_epochs=2)
        self.best_pred = 0.0
    ...
    def training(self, epoch):
        ...
        for i, dt in enumerate(self.trainloader):
    ...
    def validation(self, epoch):
        ...
        self.model.eval()
        total_inter, total_union, total_correct, total_label, total_loss = 0, 0, 0, 0, 0
        for i, dt in enumerate(self.valloader):
            image, dep, target = dt['image'], dt['depth'], dt['label'].long()
            image, dep, target = image.to(self.device), dep.to(self.device), target.to(self.device)
            with torch.no_grad():
