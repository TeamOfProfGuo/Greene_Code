...
class IRB_Decoder(nn.Module):
    def __init__(self, n_classes, feat='l', fuse_type = '1stage', mrfs = None, aux=None, auxl = None, dan=None):
        super().__init__()
        ...
        # Refine Blocks
        self.refine3 = Fuse_Block(decode_feat[3], shape=shapes[3], mmf_att=mrf_att, fuse_type=fuse_type, **self.mrf_args)
        self.refine2 = Fuse_Block(decode_feat[2], shape=shapes[2], mmf_att=mrf_att, fuse_type=fuse_type, **self.mrf_args)
        self.refine1 = Fuse_Block(decode_feat[1], shape=shapes[1], mmf_att=mrf_att, fuse_type=fuse_type, **self.mrf_args)
    ...
    def forward(self, in_feats):
        ...
        y3, _ = self.refine3(y4u, f3)   # [B, 256, 30, 30]
        y3u, y3a = self.up3(y3)         # [B, 128, 60, 60],  [B, 128, 30, 30]
        y2, _ = self.refine2(y3u, f2)
        y2u, y2a = self.up2(y2)         # [B, 64, 120, 120], [B, 64, 60, 60]
        y1, _ = self.refine1(y2u, f1)   # [B, 64, 120, 120]
        if self.dan == '21af':
            y1 = self.fuse21(y2, y1)
